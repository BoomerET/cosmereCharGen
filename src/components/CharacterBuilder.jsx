...
export default function CharacterBuilder() {
  ...
  // ===== Enhanced Fantasy Grounds XML export =====
  const escapeXML = (s = "") =>
    String(s)
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
  ...
  // Numeric carry/lift to mirror your StatsTab thresholds
  const carryMax = (str) => {
    if (str === 0) return 50;
    if (str <= 2) return 100;
    if (str <= 4) return 250;
  ...
  // normalize “Listeners” -> “Listener” like your earlier lock logic
  ...
  const buildFGXML = () => {
    ...
    // Paths node (mirrors your sample with a single path entry)
    ...
    // Ancestry block like sample (with placeholder shortcut/text)
    const ancestryName = `${char.ancestry || "Human"}${
      char.cultures?.length ? ` (${escapeXML(char.cultures[0])})` : ""
    }`;

    const totalsRanks = sumSkillRanks();
    const totalTalents = 0; // you can wire this to real talents later
    const tier = 1; // base tier for new PCs
