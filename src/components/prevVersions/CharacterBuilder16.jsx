...
export default function CharacterBuilder() {
  const [char, setChar] = useState({
    playerName: '', characterName: '', ancestry: 'Human', cultures: [], startingPath: 'Agent',
    strength:0, speed:0, intellect:0, willpower:0, awareness:0, presence:0,
    skills: SKILL_LIST.reduce((o,{name})=>({...o,[name]:0}),{}), surges:[], radiancePowers:[]
  });
  ...
  const handleHeaderChange = e => {
    ...
    const val = multiple ? Array.from(selectedOptions).map(o=>o.value).slice(0,2) : value;
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-4">Create your Cosmere RPG PC</h1>
      ...
      <div className="flex border-b mb-4">
        {TAB_CONFIG.map(tabItem => (
          <button
            ...
            className={`px-4 py-2 ${
              tab === tabItem.id
                ? 'border-b-2 border-blue-800 text-blue-500'
                : 'text-white'
            }`}
          >
            {tabItem.label}
