    ...
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="flex border-b mb-4">
        <button
          ...
          className={`${tab === 'stats' ? 'border-b-2 border-blue-500' : ''} px-4 py-2`}
        >
          Attributes/Derived Stats
        </button>
        <button
          onClick={() => setTab('abilities')}
          className={`${tab === 'abilities' ? 'border-b-2 border-blue-500' : ''} px-4 py-2`}
        >
          Skills
        </button>
      </div>
      ...
      {tab === 'stats' ? (
        ...
        <>  {/* Abilities Tab */}
          ...
          <div className="mb-6">
            ...
            {char.surges.map((s, idx) => (
              <div key={idx} className="flex items-center mb-2">
                <input
                  className="flex-1 border rounded p-1"
                  value={s}
                  onChange={e => updateListItem('surges', idx, e.target.value)}
                  placeholder="Surge Name"
                />
                <button onClick={() => removeListItem('surges', idx)} className="ml-2 text-red-500">×</button>
              </div>
            ))}
            <button onClick={() => addListItem('surges')} className="px-3 py-1 bg-blue-500 text-white rounded">Add Surge</button>
          </div>
          {/* Radiant Powers Section */}
          <div>
            <h2 className="text-xl font-semibold mb-2">Radiant Powers</h2>
            {char.radiancePowers.map((p, idx) => (
              <div key={idx} className="flex items-center mb-2">
                <input
                  className="flex-1 border rounded p-1"
                  value={p}
                  onChange={e => updateListItem('radiancePowers', idx, e.target.value)}
                  placeholder="Power Name"
                />
                <button onClick={() => removeListItem('radiancePowers', idx)} className="ml-2 text-red-500">×</button>
              </div>
            ))}
            <button onClick={() => addListItem('radiancePowers')} className="px-3 py-1 bg-blue-500 text-white rounded">Add Radiant Power</button>
          </div>
        </>
      )}
    </div>
