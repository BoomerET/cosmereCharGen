...
export default function CharacterBuilder() {
  ...
  const modifier = char[base] + rank;
  return (
    <div key={name} className="flex justify-between items-center mb-2">
      <span className="font-medium">
        {name} ({ATTR_ABBREV[base]}: {char[base]})
      </span>
      <div className="flex items-center space-x-2">
        <span className="w-12 text-center">Mod: {modifier}</span>
        {[1,2,3,4,5].map(i => (
          <input
            key={i}
            type="checkbox"
          ...
          <div className="mb-6">
            ...
            {char.surges.map((surge, idx) => (
              <div key={idx} className="flex items-center mb-2">
                ...
                />
                <button
                  onClick={() => removeListItem('surges', idx)}
          ...
          <div>
            ...
            {char.radiancePowers.map((power, idx) => (
              <div key={idx} className="flex items-center mb-2">
                <input
                  className="flex-1 border rounded p-1"
                  value={power}
                ...
                />
                <button
                  onClick={() => removeListItem('radiancePowers', idx)}
                  className="ml-2 text-red-500"
                >
                  ×
                </button>
            ...
            <button
              onClick={() => addListItem('radiancePowers')}
              className="px-3 py-1 bg-blue-500 text-white rounded"
            >
              Add Radiant Power
            </button>
