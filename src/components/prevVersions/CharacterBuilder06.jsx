// components/CharacterBuilder.jsx
...
export default function CharacterBuilder() {
  ...
  // Derived functions omitted for brevity reuse previous code
  ...
  const changeSkill=(nm,rk,chk)=>setChar(p=>{const cur=p.skills[nm];let nr=cur;if(chk&&rk===cur+1)nr=rk;else if(!chk&&rk===cur)nr=cur-1;else return p;return{...p,skills:{...p.skills,[nm]:nr}};});
  const addList=(field)=>setChar(p=>({...p,[field]:[...p[field],'']}));
  const updateList=(field,i,v)=>setChar(p=>{const a=[...p[field]];a[i]=v;return{...p,[field]:a};});
  const removeList=(field,i)=>setChar(p=>({...p,[field]:p[field].filter((_,j)=>j!==i)}));
    ...
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="flex border-b mb-4">
        {['stats','abilities'].map(t=>(
          <button key={t} onClick={()=>setTab(t)} className={`${tab===t?'border-b-2 border-blue-500':''} px-4 py-2`}>{t==='stats'?'Stats':'Abilities'}</button>
        ))}
      </div>
      {/* Tab Content */}
      {tab==='stats' ? (
        ...
        <>
          ...
          <div className="mb-6">
            ...
            <select multiple value={char.cultures} onChange={handleCulture} className="w-full border rounded p-2">
              ...
              return (
                <div key={name} className="flex justify-between items-center mb-2">
                  ...
                  <div className="flex space-x-1">
                    {[1,2,3,4,5].map(i=>(
                      <input key={i} type="checkbox" checked={i<=rk} disabled={!((i===rk+1&&rk<5)||(i===rk&&rk>0))} onChange={e=>changeSkill(name,i,e.target.checked)} className={`w-4 h-4 border rounded ${i<=rk?'bg-blue-500':''}`}/>
                    ))}
                  </div>
                </div>
              );
          ...
          <div className="mb-6">
            ...
            {char.surges.map((s,i)=>(
              <div key={i} className="flex items-center mb-2">
                <input value={s} onChange={e=>updateSurge(i,e.target.value)} placeholder="Surge Name" className="flex-1 border rounded p-1"/>
                <button onClick={()=>removeSurge(i)} className="ml-2 text-red-500">×</button>
              </div>
            ))}
            <button onClick={()=>addSurge()} className="px-3 py-1 bg-blue-500 text-white rounded">Add Surge</button>
          </div>
          {/* Radiant Powers */}
          <div>
            <h2 className="text-xl font-semibold mb-2">Radiant Powers</h2>
            {char.radiancePowers.map((p,i)=>(
              <div key={i} className="flex items-center mb-2">
                <input value={p} onChange={e=>updateRadiance(i,e.target.value)} placeholder="Power Name" className="flex-1 border rounded p-1"/>
                <button onClick={()=>removeRadiance(i)} className="ml-2 text-red-500">×</button>
              </div>
            ))}
            <button onClick={()=>addRadiance()} className="px-3 py-1 bg-blue-500 text-white rounded">Add Radiant Power</button>
          </div>
        </>
      )}
