...
export default function CharacterBuilder() {
  ...
  const changeSkillRank = (skillName, rank, checked) => setChar(prev => {
    ...
    let newRanks = current;
    if (checked && rank === current + 1) newRanks = rank;
    else if (!checked && rank === current) newRanks = current - 1;
    else return prev;
    return { ...prev, skills: { ...prev.skills, [skillName]: newRanks } };
  });
  const addSurge = () => setChar(prev => ({ ...prev, surges: [...prev.surges, ''] }));
  const updateSurge = (i, v) => setChar(prev => {
    const arr = [...prev.surges]; arr[i] = v; return { ...prev, surges: arr };
  });
  ...
  const updateRadiance = (i, v) => setChar(prev => {
    const arr = [...prev.radiancePowers]; arr[i] = v; return { ...prev, radiancePowers: arr };
  });
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block font-medium">Ancestry</label>
          <select
            name="ancestry"
            value={char.ancestry}
            onChange={handleChange}
        ...
        <div>
          <label className="block font-medium">Culture (up to 2)</label>
          <select
            multiple
            name="cultures"
            value={char.cultures}
        ...
        <div>
          <label className="block font-medium">Starting Path</label>
          <select
            name="startingPath"
      ...
      <div className="mt-6">
        <h2 className="text-2xl font-semibold mb-2">Distribute Attributes (0–3, total 12)</h2>
        {stats.map(stat => (
          <div key={stat} className="flex items-center space-x-4 mb-2">
            <label className="w-32 font-medium capitalize">{stat}</label>
            <button
              type="button"
            ...
            >-</button>
            <span className="w-8 text-center">{char[stat]}</span>
            <button
              type="button"
              onClick={() => changeStat(stat, 1)}
      ...
      {/* Derived Stats */}
      <div className="mt-6">
        <h2 className="text-2xl font-semibold mb-2">Derived Stats</h2>
      ...
      <div className="mt-6">
        ...
        {SKILL_LIST.map(({ name, base }) => {
          const rank = char.skills[name];
          return (
            <div key={name} className="mb-4">
              <div className="flex justify-between items-center">
                <span className="font-medium">{name} (Base: {capitalize(base)}: {char[base]})</span>
                <div className="flex space-x-1">
                  {[1,2,3,4,5].map(i => (
                    <input
                      ...
                      type="checkbox"
                      checked={i <= rank}
                      disabled={!((i === rank + 1 && rank < 5) || (i === rank && rank > 0))}
                      onChange={e => changeSkillRank(name, i, e.target.checked)}
                      className={`w-4 h-4 border rounded ${i <= rank ? 'bg-blue-500' : ''}`}
                    />
                  ))}
                </div>
              </div>
      ...
      {/* Surges */}
      <div className="mt-6">
        <h2 className="text-2xl font-semibold mb-2">Surges</h2>
        {char.surges.map((surge, idx) => (
          <div key={idx} className="flex items-center space-x-2 mb-2">
            <input
              type="text"
              placeholder="Surge Name"
              value={surge}
              onChange={e => updateSurge(idx, e.target.value)}
      ...
      {/* Radiant Powers */}
      <div className="mt-6">
        <h2 className="text-2xl font-semibold mb-2">Radiant Powers</h2>
        {char.radiancePowers.map((rp, idx) => (
          <div key={idx} className="flex items-center space-x-2 mb-2">
            <input
              type="text"
              placeholder="Power Name"
              value={rp}
              onChange={e => updateRadiance(idx, e.target.value)}
              className="w-full border rounded p-1"
            />
            <button
              type="button"
              onClick={() => removeRadiance(idx)}
              className="text-red-500"
            >Remove</button>
          </div>
        ))}
        <button
          ...
          onClick={addRadiance}
          className="mt-2 px-3 py-1 bg-blue-500 text-white rounded"
        >Add Radiant Power</button>
      </div>

      {/* Summary */}
      <div className="mt-8 p-4 border rounded bg-gray-800 text-white">
        <h2 className="text-2xl font-semibold mb-2">Character Summary</h2>
